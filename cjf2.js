...
const { TwitterApi } = require('twitter-api-v2');       require('dotenv').config();
                                                        // Load replies from replies.json (safely)              let replies = [];
try {                                                     const repliesPath = path.join(__dirname, 'replies.json');
  if (fs.existsSync(repliesPath)) {                         const data = fs.readFileSync(repliesPath, 'utf-8').trim();
    if (data) {                                               const parsed = JSON.parse(data);                        if (Array.isArray(parsed) && parsed.length > 0) {
        replies = parsed;
      } else {
...
const ZEALY_URL = 'https://api-v1.zealy.io/communities/travala/questboard/v2?filters=locked&filters=available&filters=inCooldown&filters=inReview&filters=completed';
const QUEST_URL = 'https://api-v1.zealy.io/communities/travala/quests/v2';
const CLAIM_URL = 'https://api-v1.zealy.io/communities/travala/quests/v2';
const INTERVAL = 300;
const REQUEST_DELAY = 150;
const FILE_PATH = path.join(__dirname, 'asta.json');
const MY_USERNAME = 'BlakeJames27870';
const IS_X_AUTH_CONFIGURED = false;
const ZEALY_COOKIE = 'access_token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiIyNzA1NjI1My0wM2MwLTRkOWQtOTcxMS1hOWM3YzRmMmMwMWQiLCJhY2NvdW50VHlwZSI6ImVtYWlsIiwiZW1haWwiOiJmZWxpeG5tZW5pNTZAZ21haWwuY29tIiwibGFzdEVtYWlsQ2hlY2siOjE3NzExOTQ5NjE1NDksImlhdCI6MTc3MTE5NDk2MSwiZXhwIjoxNzczNzg2OTYxfQ._46-C-OBwcRw55lLUodf6cv9ciUYjiBeZy3r9_skOgM; user_metadata=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiIyNzA1NjI1My0wM2MwLTRkOWQtOTcxMS1hOWM3YzRmMmMwMWQiLCJpYXQiOjE3NzExOTQ5NjEsImV4cCI6MTc3Mzc4Njk2MX0.ELKw0YBnOWKNDT1tygsPM0qK3Qmnt9OVr3QPaeGfDyw';
        ...
        while (shouldRetry) {
          ...
          for (const task of taskResults) {
            ...
            if (type === 'tweetReact') {
              ...
              claimTasks.push({ taskId, type, value: "frozen" });
            } else if (type === 'file') {
